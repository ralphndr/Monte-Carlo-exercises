...
```bash
git clone https://github.com/ralphndr/Monte-Carlo-exercises.git
...
| RANDU Hyperplanes | Ising Model | Label Switching |
|:-:|:-:|:-:|
| ![RANDU](img/randu_hyperplanes.png) | ![Ising](img/ising_grid.png) | ![Label switching](img/label_switching.png) |
...
Analysis of the infamous RANDU linear congruential generator. We prove analytically that the triples (u_{i-2}, u_{i-1}, u_i) lie on exactly **15 parallel hyperplanes**, then verify it empirically via 3D scatter plots and bit-pattern analysis. Compared against Python's Mersenne Twister.

**Key result:** u_i = 6*u_{i-1} - 9*u_{i-2} + k, with k in {-5, ..., 9}

---

### 2 · Laplace Distribution & Rejection Sampling
- Inverse CDF derivation and implementation of a Laplace generator
- Rejection sampler for N(0,1) using Laplace as proposal
- Optimal constant c = sqrt(2e/pi) ≈ 1.316, acceptance rate ≈ 76%
- Discussion of why sampling Laplace from Normal is impossible (tail behaviour)

---

### 3 · Improved Box-Muller (Polar Method)
Proof that the polar Box-Muller algorithm produces exact N(0,1) samples via a polar coordinate argument. Performance comparison against standard Box-Muller (trigonometric vs rejection cost).
...
| Inverse CDF: ceil(log U / log(1-p)) | O(1) |
...
Computing P(X in A) where X ~ N_d(0, I_d) and A = {|prod x_i| <= c}.
...
Critique of a naive adaptive IS scheme (circular dependency). Fix via **two-batch adaptive IS**: use one batch to estimate the proposal N(mu_hat, Sigma_hat), a fresh batch to compute self-normalized weights.
...
Gibbs sampler for p(x) proportional to exp(alpha * sum(x_i) + beta * sum_{i~j} 1[x_i=x_j]).

- Full conditional: x_i | x_{-i} ~ Bernoulli(sigmoid(alpha + beta*(n1 - n0)))
- Mixing diagnostics: magnetization traces, autocorrelation, grid snapshots
- **Critical value:** beta_c = ln(1+sqrt(2)) ≈ 0.88 — phase transition beyond which mixing collapses

---

### 9 · Hierarchical Bayesian Model
Model: y_i|theta_i ~ Bin(n_i, theta_i), theta_i ~ Beta(alpha, beta), alpha/beta ~ Gamma(a, b).

- theta_i | rest ~ Beta(y_i + alpha, n_i - y_i + beta) — conjugate
- alpha, beta | rest — non-standard, MH within Gibbs
- **Debugging trick:** freeze variables one at a time to isolate bugs (single-component debugging)

---

### 10 · Gaussian Mixture Models
Gibbs sampler with latent variables z_i for K-component mixture (1/K) * sum phi(y; mu_k, 1).
...
Black-Scholes European put: V = E[exp(-rT) * (K - S(T))^+].

- **Q11.1:** Brownian bridge — W_t | W_a, W_b ~ N(((b-t)*W_a + (t-a)*W_b) / (b-a), (t-a)*(b-t)/(b-a))
...
At each iteration: sample from N(mu, Sigma), keep elite fraction rho, refit distribution. Exponential convergence toward x* = (1,...,1), tested for d in {2, 5, 10}.
...
- The critical temperature in Ising (beta_c ≈ 0.88) marks a phase transition where MCMC mixing collapses
