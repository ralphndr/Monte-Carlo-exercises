...
Proof that the polar Box-Muller algorithm produces exact $\mathcal{N}(0,1)$ samples via a polar coordinate argument. Performance comparison against standard Box-Muller (trigonometric vs rejection cost).
...
CE algorithm applied to the **Rosenbrock function** $S(x) = \sum_{i=1}^{d-1} [100(x_{i+1}-x_i^2)^2 + (x_i-1)^2]$.
